...
--======================================================================================
-- MODULE: StandReach (v2 - Character-Parented)
-- PURPOSE: A specialized reach script for Stands that are parented directly to the
--          player's character. It directly finds the Stand model within the character
--          and resizes its weapon's primary damage part.
-- ARCHITECTURE: Abandons workspace scanning for a much more efficient and reliable
--               direct child search within the LocalPlayer's Character.
--======================================================================================
...
Modules.StandReach = {
    State = {
        OriginalSizes = {} -- Maps a Part instance to its original Vector3 size.
    }
}

--- Finds the Stand model directly inside the player's character.
-- @return The Stand Model instance, or nil if not found.
function Modules.StandReach:_findPlayerStand()
    local myCharacter = LocalPlayer.Character
    if not myCharacter then return nil end

    -- Iterate through the direct children of the player's character.
    for _, child in ipairs(myCharacter:GetChildren()) do
        -- A Stand is a Model that contains a Tool. This is a reliable way to identify it.
        if child:IsA("Model") and child:FindFirstChildOfClass("Tool") then
            return child -- Return the first model that fits the criteria.
        end
    end
    
    return nil -- Return nil if no Stand model was found.
end

--- Applies the hitbox extension to the Stand's weapon.
-- @param size The new uniform size for the hitbox part.
function Modules.StandReach:Apply(size)
    local standModel = self:_findPlayerStand()
    if not standModel then
        return DoNotif("Your Stand is not currently summoned or could not be found.", 4)
    end

    local tool = standModel:FindFirstChildOfClass("Tool")
    if not tool then
        return DoNotif("Found Stand '"..standModel.Name.."', but it has no tool.", 4)
    end
    
    -- We prioritize finding a part named "Blade", as it's the most likely damage part.
    local damagePart = tool:FindFirstChild("Blade")
    if not damagePart or not damagePart:IsA("BasePart") then
        return DoNotif("Found Tool '"..tool.Name.."', but could not find its 'Blade' part.", 4)
    end
    ...
    damagePart.Transparency = 0.5 -- Semi-visible to confirm it worked.

    DoNotif("Extended '" .. damagePart.Name .. "' on '" .. standModel.Name .. "' to size " .. size .. ".", 4)
end

--- Restores the original sizes of any modified Stand weapon parts.
function Modules.StandReach:Reset()
    if not next(self.State.OriginalSizes) then
        return DoNotif("No active Stand reach modifications to reset.", 3)
    end
    ...
    for part, originalSize in pairs(self.State.OriginalSizes) do
        if part and part.Parent then
            ...
            part.Massless = false
            part.Transparency = 1 -- Restore original transparency
    ...
    if resetCount > 0 then
        DoNotif("Reset " .. resetCount .. " Stand hitbox(es).", 3)
    else
        DoNotif("Could not find previously modified parts to reset.", 3)
    end
end

--======================================================================================
-- COMMAND REGISTRATION (Usage remains the same)
--======================================================================================
RegisterCommand({
    Name = "standreach",
    Aliases = {"sr", "standhitbox"},
    Description = "Extends the hitbox of your currently summoned Stand's weapon. Usage: standreach [size]"
}, function(args)
...
RegisterCommand({
    Name = "unstandreach",
    Aliases = {"unsr", "resetstand"},
    Description = "Resets your Stand's weapon hitbox to its original size."
}, function(args)
